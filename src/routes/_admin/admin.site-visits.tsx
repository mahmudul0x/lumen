...
function makeKindPage(kind: string, title: string, description: string) {
  function Page() {
    const [items, setItems] = useState<any[]>([]);
    const [loading, setLoading] = useState(true);
    useEffect(() => {
      supabase.from("leads").select("*").eq("kind", kind).order("created_at", { ascending: false }).limit(200).then(({ data }) => {
        setItems(data ?? []); setLoading(false);
      });
    }, []);
    return (
      <AdminPage title={title} description={`${description} • মোট ${items.length}`}>
        <AdminCard className="!p-0 overflow-hidden">
          {loading ? <div className="grid h-40 place-items-center"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
            : items.length === 0 ? <EmptyState title="কোনো ডাটা নেই" hint="নতুন এন্ট্রি আসলে এখানে দেখাবে" />
            : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 text-left text-xs uppercase tracking-wider text-muted-foreground">
                    <tr><th className="p-3">আইডি</th><th className="p-3">গ্রাহক</th><th className="p-3">ফোন</th><th className="p-3">প্রকল্প</th><th className="p-3">স্ট্যাটাস</th><th className="p-3">তারিখ</th></tr>
                  </thead>
...
export const Route = createFileRoute("/_admin/admin/site-visits")({
  component: makeKindPage("book-visit", "সাইট ভিজিট", "বুকিং ও অ্যাসাইনমেন্ট"),
});
